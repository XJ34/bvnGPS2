...
clf1 = RandomForestClassifier(n_estimators=108, class_weight='balanced')
clf1.fit(health_data, label_h, )

health_result = clf1.feature_importances_
bac_result = clf2.feature_importances_
virus_result = clf3.feature_importances_
bacn_result = clf4.feature_importances_
for i in health_result:
    with open('rf_coe1.txt', 'a', encoding='utf-8') as h:
        h.write(str(abs(i)))
        h.write('\n')
for j in bac_result:
    with open('coe rf2.txt', 'a', encoding='utf-8') as b:
        b.write(str(abs(j)))
        b.write('\n')
for z in virus_result:
    with open('coe rf3.txt', 'a', encoding='utf-8') as v:
        v.write(str(abs(z)))
        v.write('\n')
for x in bacn_result:
    with open('coe rf4.txt', 'a', encoding='utf-8') as bn:
        bn.write(str(abs(x)))
        bn.write('\n')
